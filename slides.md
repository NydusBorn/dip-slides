...
- Изображения схем представлены в разрешении 20 мегапикселей.
- На схемах размечаются конкретные УГО и УТО:
  - Для УГО выделяется такой объект, который полностью представляет собой артикул.
  - Для УТО выделяется весь текст относящийся к УГО.
...
- Изображение сжимается до 1024 пикселей, и сохраняется в JPG формате, для уменьшения размера изображений на диске.
...
# Распознавание изображения (Бэкенд)
...
Также стоит отметить что для работы моделей они полностью задействуют ресурсы GPU, так как это требуется для максимального 
разрешения на этапе выделения объектов.

--- 

# Распознавание изображения (Фронтенд)

После передачи данных с бэкенда bbox выглядит следующим образом:
